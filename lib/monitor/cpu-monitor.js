...
class CPUMonitor {
  ...
  evaluate(cli) {
    ...
    if (this.platform == 'darwin') {
      ...
      */
      const cpus = cli.toString('UTF8').split(' ');
      const cpuUser = cpus[2];
      const cpuNice = cpus[3];
      const cpuSys = cpus[4];
      const cpuIdle = cpus[5];
      const cpuIOWait = cpus[6];
      const cpuIRQ = cpus[7];
      const cpuSoftIRQ = cpus[8];

      const sum = cpuUser + cpuNice + cpuSys + cpuIdle + cpuIOWait + cpuIOWait + cpuIRQ + cpuSoftIRQ;

      cpu = { user: cpuUser / sum * 100, sys: cpuSys / sum * 100, idle: cpuIdle / sum * 100 };
    }
    return cpu;
  }
