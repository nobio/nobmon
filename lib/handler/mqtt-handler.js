...
class MQTTHander {
  constructor() {
    ...
    this.mqttClient = mqtt.connect(mqtt_uri, options);
    console.log(`connecting to mqtt host ${mqtt_uri} DONE! (connected: ${this.mqttClient.connected})`);
  }
  ...
  */
  send(data) {
    if (!this.mqttClient.connected) { console.log(`not connected to mqtt host ${mqtt_uri}...`); return; }
