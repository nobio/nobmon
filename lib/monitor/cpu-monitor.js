  ...
  cpu = async () => {
    ...
    try {
      ...
      if (this.platform == 'darwin') {
        // cmd = 'top -l 1 | grep "CPU usage" | awk -F " " \'{print $3}\' | awk -F "%" \'{print $1}\'';  // CPU usage: 6.59% user, 24.17% sys, 69.23% idle
        cmd = 'top -l 1 | grep "CPU usage"';  // CPU usage: 6.59% user, 24.17% sys, 69.23% idle
      } else if (this.platform == 'linux') {
        // cmd = 'export TERM=xterm; top -n 1 | grep Cpu | awk -F " " \'{print $2}\'';          // %Cpu(s):  3.7 us,  3.7 sy,  0.0 ni, 92.7 id,  0.0 wa,  0.0 hi,  0.0 si,  0.0 st
