...
export class CPUMonitor {
  ...
  evaluate(cli) {
    ...
    if (this.platform === 'darwin') {
      ...
      */
      const cpus = cli.toString('UTF8').split(' ');
      const cpuUser = parseInt(cpus[2], 10, 10);
      const cpuNice = parseInt(cpus[3], 10);
      const cpuSys = parseInt(cpus[4], 10);
      const cpuIdle = parseInt(cpus[5], 10);
      const cpuIOWait = parseInt(cpus[6], 10);
      const cpuIRQ = parseInt(cpus[7], 10);
      const cpuSoftIRQ = parseInt(cpus[8], 10);
  ...
  async monitor() {
    ...
    console.log(`${new Date().toLocaleString()} | monitoring cpu: usage ${cpu.usage}% user ${cpu.user}%, sys ${cpu.sys}%, idle ${cpu.idle}%`);

    const data = [];
    data.push({
      name: 'cpu-usage', topic: 'cpu_load', type: 'float', value: cpu.usage,
    });
    data.push({
      name: 'cpu-user', topic: 'cpu_load', type: 'float', value: cpu.user,
    });
    data.push({
      name: 'cpu-sys', topic: 'cpu_load', type: 'float', value: cpu.sys,
    });
    data.push({
      name: 'cpu-idle', topic: 'cpu_load', type: 'float', value: cpu.idle,
    });
