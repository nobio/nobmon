...
export class MemoryMonitor {
  ...
  /** implemented interface 'run'; will be called from main thread */
  run = async () => {
    setInterval(async () => {
      try {
        this.monitor();
  ...
  async monitor() {
    ...
    const data = [];
    data.push({
      name: 'mem-total', topic: 'memory', type: 'int', value: memory.total,
    });
    data.push({
      name: 'mem-used', topic: 'memory', type: 'int', value: memory.used,
    });
