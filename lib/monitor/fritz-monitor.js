...
export class FritzboxMonitor {
  ...
  constructor(handlers) {
    ...
    // constants
    this.FRITZ_URL = process.env.FRITZ_URL;
  ...
  getCurrentRates = async () => {
    ...
    try {
      const xml = execSync(`curl --max-time ${this.CURL_TIMEOUT} http://${this.FRITZ_URL}:49000/igdupnp/control/${URL} \
  ...
  getCommonInfo = async () => {
    ...
    try {
      ...
      console.error(`{"Connection":"ERROR - Could not retrieve status from FRITZ!Box"} ${error}`);
      console.error(error);
      return null;
    }
  };
  ...
  sendRequest = async () => {
    ...
    const resp = {
      downloadRate: currentRates.NewByteReceiveRate[0] * 8,
      uploadRate: currentRates.NewByteSendRate[0] * 8,
      downloadTotal: currentRates.NewTotalBytesReceived[0] * 8,
      uploadTotal: currentRates.NewTotalBytesSent[0] * 8,
      downloadMax: this.maxDownload,
      uploadMax: this.maxUpload,
    };
  ...
  async monitor() {
    try {
      ...
      // console.log(`${new Date().toLocaleString()} | monitoring Fritzbox ${JSON.stringify(data)}`);
